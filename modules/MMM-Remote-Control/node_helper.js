...
var defaultModules = require(path.resolve(
  __dirname + "/../default/defaultmodules.js"
));
...
Module = {
  ...
  register: function (name, moduleDefinition) {
    ...
    Module.notificationHandler[name] =
  ...
  Object.assign(
    {
      ...
      start: function () {
        ...
        fs.readFile(
          path.resolve(__dirname + "/remote.ejs"),
          function (err, data) {
      ...
      combineConfig: function () {
        ...
        try {
          ...
          if ("modules" in this.configOnHd) {
            let thisModule = this.configOnHd.modules.find(
      ...
      createRoutes: function () {
        ...
        this.expressApp.get("/remote.ejs", function (req, res) {
          if (self.template === "") {
            res.send(503);
          } else {
            res.contentType("text/html");
            res.set("frame-ancestors http://*:*");
        ...
        // this.expressApp.get("/updateProfileList", function(req, res) {
        //     var query = url.parse(req.url, true).query;

        //     res.render('remote,ejs', {title:'Express'})
      ...
      readModuleData: function () {
        ...
        fs.readFile(path.resolve(__dirname + "/modules.json"), (err, data) => {
          ...
          for (let i = 0; i < self.modulesAvailable.length; i++) {
            self.modulesAvailable[i].name = self.formatName(
              self.modulesAvailable[i].longname
            );
          ...
          for (let i = 0; i < defaultModules.length; i++) {
            ...
            var modulePath =
              self.configOnHd.paths.modules + "/default/" + defaultModules[i];
            self.loadModuleDefaultConfig(
              module,
              modulePath,
              i === defaultModules.length - 1
            );
          }

          // now check for installed modules
          fs.readdir(path.resolve(__dirname + "/.."), function (err, files) {
            let installedModules = files.filter(
              (f) => ["node_modules", "default", "README.md"].indexOf(f) === -1
            );
      ...
      loadModuleDefaultConfig: function (module, modulePath, lastOne) {
        ...
        try {
          ...
          if (e.code == "ENOENT") {
            console.error(
              "ERROR! Could not find main module js file for " + module.longname
            );
          } else if (e instanceof ReferenceError || e instanceof SyntaxError) {
            ...
            console.error(
              "ERROR! Could not load main module js file. Error found: " + e
            );
          }
        }
        if (lastOne) {
      ...
      removeDefaultValues: function (config) {
        ...
        delete require.cache[
          require.resolve(__dirname + "/../../js/defaults.js")
        ];
        ...
        for (let key in defaultConfig) {
          if (
            ...
            _.isEqual(defaultConfig[key], config[key])
          ) {
            delete config[key];
          }
        }
        ...
        for (let i = 0; i < config.modules.length; i++) {
          ...
          for (let key in def) {
            if (
              ...
              current.config.hasOwnProperty(key) &&
              _.isEqual(def[key], current.config[key])
            ) {
      ...
      answerPost: function (query, req, res) {
        ...
        if (query.data === "config") {
          ...
          if (best === -1) {
            ...
            self.sendResponse(
              res,
              new Error("Backing up config failed, not saving!"),
              { query: query }
            );
            return;
          }
      ...
      monitorControl: function (action, opts, res) {
        ...
        let monitorOnCommand =
          this.initialized &&
          "monitorOnCommand" in this.thisConfig.customCommand
            ? this.thisConfig.customCommand.monitorOnCommand
            : "vcgencmd display_power 1";
        let monitorOffCommand =
          this.initialized &&
          "monitorOffCommand" in this.thisConfig.customCommand
            ? this.thisConfig.customCommand.monitorOffCommand
            : "vcgencmd display_power 0";
        let monitorStatusCommand =
          this.initialized &&
          "monitorStatusCommand" in this.thisConfig.customCommand
      ...
      shutdownControl: function (action, opts, res) {
        let shutdownCommand =
          this.initialized && "shutdownCommand" in this.thisConfig.customCommand
            ? this.thisConfig.customCommand.shutdownCommand
            : "sudo shutdown -h now";
        let rebootCommand =
          this.initialized && "rebootCommand" in this.thisConfig.customCommand
            ? this.thisConfig.customCommand.rebootCommand
            : "sudo shutdown -r now";
        if (action === "SHUTDOWN") {
          exec(shutdownCommand, opts, (error, stdout, stderr, res) => {
            this.checkForExecError(error, stdout, stderr, res);
          });
        }
        if (action === "REBOOT") {
          exec(rebootCommand, opts, (error, stdout, stderr, res) => {
            this.checkForExecError(error, stdout, stderr, res);
          });
        }
      },
      ...
      executeQuery: function (query, res) {
        ...
        if (query.action === "COMMAND") {
          if (
            this.thisConfig.customCommand &&
            this.thisConfig.customCommand[query.command]
          ) {
            exec(
              this.thisConfig.customCommand[query.command],
              opts,
        ...
        if (
          ["MONITORON", "MONITOROFF", "MONITORTOGGLE", "MONITORSTATUS"].indexOf(
            query.action
          ) !== -1
        ) {
        ...
        if (query.action === "MANAGE_CLASSES") {
          if (
            !query.payload ||
            !query.payload.classes ||
            !this.thisConfig ||
            !this.thisConfig.classes
          )
            return;
          let classes = [];
          switch (typeof query.payload.classes) {
            case "string":
              classes.push(this.thisConfig.classes[query.payload.classes]);
              break;
            case "object":
              query.payload.classes.forEach((t) =>
                classes.push(this.thisConfig.classes[t])
              );
          }
          classes.forEach((cl) => {
            for (const act in cl) {
        ...
        if (
          ["MINIMIZE", "TOGGLEFULLSCREEN", "DEVTOOLS"].indexOf(query.action) !==
          -1
        ) {
          try {
      ...
      installModule: function (url, res, data) {
        ...
        simpleGit(path.resolve(__dirname + "/..")).clone(
          url,
          path.basename(url),
                ...
                (error, stdout, stderr) => {
                  if (error) {
                    ...
                    self.sendResponse(
                      res,
                      error,
                      Object.assign({ stdout: stdout, stderr: stderr }, data)
                    );
                  } else {
                    // success part
                    self.readModuleData();
                    self.sendResponse(
                      res,
                      undefined,
                      Object.assign({ stdout: stdout }, data)
                    );
                  }
                }
              );
      ...
      updateModule: function (module, res) {
        ...
        if (typeof module !== "undefined" && module !== "undefined") {
          if (self.modulesAvailable) {
            var modData = self.modulesAvailable.find(
              (m) => m.longname === module
            );
      ...
      controlPm2: function (res, query) {
        ...
        var pm2 = require("pm2");
        let processName =
          query.processName || this.thisConfig.pm2ProcessName || "mm";
        ...
        fs.readFile(
          ...
          function (err, data) {
            if (err) {
              ...
              self.translation = Object.assign(
                {},
                self.translation,
      ...
      loadCustomMenus: function () {
        if ("customMenu" in this.thisConfig) {
          let menuPath = path.resolve(
            __dirname + "/../../config/" + this.thisConfig.customMenu
          );
          if (!fs.existsSync(menuPath)) {
            console.log(
              `MMM-Remote-Control customMenu Requested, but file:${menuPath} was not found`
            );
            return;
          }
          fs.readFile(menuPath, (err, data) => {
            if (err) {
              ...
              this.customMenu = Object.assign(
                {},
                this.customMenu,
                JSON.parse(this.translate(data.toString()))
              );
              this.sendSocketNotification(
                "REMOTE_CLIENT_CUSTOM_MENU",
                this.customMenu
              );
            }
          });
      ...
      socketNotificationReceived: function (notification, payload) {
        ...
        if (notification === "REQUEST_DEFAULT_SETTINGS") {
          ...
          self.sendSocketNotification(
            "LOAD_PORT",
            self.configOnHd.port ? self.configOnHd.port : ""
          );
