...
var Remote = {
        ...
        let message =
          payload.status === "error"
            ? this.translate("RESPONSE_ERROR") +
              ": <br><pre><code>" +
              JSON.stringify(payload, undefined, 3) +
              "</code></pre>"
            : payload.info;
  ...
  loadButtons: function (buttons) {
    Object.keys(buttons).forEach((key) => {
      document
        .getElementById(key)
        .addEventListener("click", buttons[key], false);
    });
  ...
  loadOtherElements: function () {
    ...
    slider.addEventListener(
      "change",
      function (event) {
        self.sendSocketNotification("REMOTE_ACTION", {
          action: "BRIGHTNESS",
          value: slider.value
        });
      },
  ...
  showMenu: function (newMenu) {
    ...
    if (this.currentMenu === "settings-menu") {
      ...
      if (changes > 0) {
        ...
        var discard = self.createSymbolText(
          "fa fa-warning",
    ...
    if (newMenu === "main-menu") {
      this.loadList("config-modules", "config", function (parent, configData) {
        ...
        if (
          !configData.modules.find((m) => m.module === "alert") &&
          alertElem !== undefined
        )
          alertElem.remove();

        let modConfig = configData.modules.find(
          (m) => m.module === "MMM-Remote-Control"
        ).config;
        let classElem = document.getElementById("classes-button");
        if ((!modConfig || !modConfig.classes) && classElem !== undefined)
          classElem.remove();
      });
    }
    ...
    if (message) {
      text =
        typeof message === "object"
          ? JSON.stringify(message, undefined, 3)
          : message;
    }
    parent.appendChild(
      this.createSymbolText("fa fa-fw " + symbol, text, onClick)
    );
  ...
  loadListCallback: function (result) {
    ...
    var loadingIndicator = document.getElementById(
      result.query.listname + "-loading"
    );
    var emptyIndicator = document.getElementById(
      result.query.listname + "-empty"
    );
    ...
    self.loadToggleButton(moduleBox, function (toggledOn, event) {
      if (toggledOn) {
        if (self.hasClass(event.currentTarget, "external-locked")) {
          ...
          warning.innerHTML = self
            .translate("LOCKSTRING_WARNING")
            .replace("LIST_OF_MODULES", visibilityStatus.modules);
          ...
          var force = self.createSymbolText(
            "fa fa-warning",
            self.translate("FORCE_SHOW"),
            (function (target) {
              return function () {
                target.className = target.className
                  .replace(" external-locked", "")
                  .replace("toggled-off", "toggled-on");
  ...
  createTypeEditSelection: function (key, parent, type, oldElement) {
    ...
    select.addEventListener(
      ...
      function (event) {
        var newType =
          select.options[select.selectedIndex].innerHTML.toLowerCase();
  ...
  createConfigLabel: function (key, name, type, forcedType, symbol) {
    ...
    var desc = Remote.createSymbolText(
      "fa fa-fw " + symbol,
    ...
    if (!forcedType) {
      var typeLabel = Remote.createSymbolText(
        "fa fa-fw fa-pencil",
        this.formatName(type),
        function (event) {
          var thisElement = event.currentTarget;
          label.replaceChild(
            self.createTypeEditSelection(key, label, type, thisElement),
            thisElement
          );
        },
        "span"
      ...
      var remove = Remote.createSymbolText(
        "fa fa-fw fa-times-circle",
        this.translate("DELETE_ENTRY"),
        function (event) {
          var thisElement = event.currentTarget;
  ...
  createConfigElement: function (type) {
    ...
    return {
      string: function (key, name, value, type, forcedType) {
        ...
        } else if (key === "<root>/classes") {
          input.placeholder = self.translate("");
        }
        return label;
      },
  ...
  createObjectGUI: function (path, name, dataToEdit) {
    ...
    if (this.createConfigElement(type)) {
      // recursion stop
      return this.createConfigElement(type)(
        path,
        name,
        dataToEdit,
        type,
    ...
    if (type === "array") {
      // array
      var add = this.createSymbolText(
        "fa fa-fw fa-plus",
        this.translate("ADD_ENTRY")
      );
      add.className += " bottom-spacing button";
      wrapper.appendChild(
        this.createConfigLabel(path, name, type, forcedType, "fa-list-ol")
      );
      wrapper.appendChild(add);
      for (var i = 0; i < dataToEdit.length; i++) {
        var newName = "#" + i;
        wrapper.appendChild(
          this.createObjectGUI(path + "/" + newName, newName, dataToEdit[i])
        );
      }
      add.addEventListener(
        ...
        function () {
          ...
          var lastType = self.getTypeAsString(
            path + "/#" + lastIndex,
          ...
          wrapper.appendChild(
            self.createObjectGUI(
              path + "/" + nextName,
              nextName,
              dataToEdit[dataToEdit.length - 1]
            )
          );
        },
        false
    ...
    if (path !== "<root>") {
      wrapper.appendChild(
        this.createConfigLabel(path, name, type, forcedType, "fa-list-ul")
      );

      var addElement = self.createConfigLabel(
        path + "/<add>",
        this.translate("ADD_ENTRY"),
        type,
        true,
      ...
      var addFunction = function () {
        ...
        var newElement = self.createObjectGUI(
          path + "/" + key,
          key,
          dataToEdit[key]
        );
    ...
    if (path === "<root>") {
      keys = ["module", "disabled", "position", "header", "classes", "config"];
    }
    for (let i = 0; i < keys.length; i++) {
      var key = keys[i];
      if (dataToEdit.hasOwnProperty(key)) {
        wrapper.appendChild(
          this.createObjectGUI(path + "/" + key, key, dataToEdit[key])
        );
      }
    }
    if (path === "<root>") {
  ...
  appendConfigMenu: function (index, wrapper) {
    ...
    var menuElement = self.createSymbolText(
      "small fa fa-fw fa-navicon",
      self.translate("MENU"),
      function (event) {
    ...
    var help = self.createSymbolText(
      "fa fa-fw fa-question-circle",
      self.translate("HELP"),
      function (event) {
        window.open(
          "config-help.html?module=" + self.currentConfig.module,
          "_blank"
        );
      }
    );
    menuDiv.appendChild(help);
    var undo = self.createSymbolText(
      "fa fa-fw fa-undo",
    ...
    var save = self.createSymbolText(
      "fa fa-fw fa-save",
      self.translate("SAVE"),
      function (event) {
    ...
    var line2 = document.createElement("classes");
    line2.className = "classes";
  ...
  createConfigPopup: function (index) {
    ...
    if (!("classes" in self.currentConfig)) {
      self.currentConfig.classes = "";
    }
  ...
  createChangedWarning: function () {
    var self = this;
    var changed = Remote.createSymbolText(
      "fa fa-fw fa-warning",
  ...
  appendModuleEditElements: function (wrapper, moduleData) {
    ...
    for (var i = 0; i < moduleData.length; i++) {
      ...
      var moduleBox = self.createSymbolText(
        "fa fa-fw fa-pencil",
        self.formatName(moduleData[i].module),
        function (event) {
      ...
      var remove = Remote.createSymbolText(
        ...
        this.translate("DELETE_ENTRY"),
        function (event) {
          var i = event.currentTarget.parentNode.firstChild.id.replace(
            "edit-module-",
            ""
          );
  ...
  createAddingPopup: function (index) {
    ...
    if (data.installed) {
      var add = self.createSymbolText(
        "fa fa-fw fa-plus",
        self.translate("ADD_THIS"),
        function (event) {
          self.closePopup();
    ...
    if (data.installed) {
      let statusElement = self.createSymbolText(
        "fa fa-fw fa-check-circle",
        self.translate("INSTALLED")
      );
      footer.appendChild(statusElement);
    } else {
      let statusElement = self.createSymbolText(
        "fa fa-fw fa-download",
        self.translate("DOWNLOAD"),
        function (event) {
    ...
    var githubElement = self.createSymbolText(
      "fa fa-fw fa-github",
      self.translate("CODE_LINK"),
      function (event) {
  ...
  offerRestart: function (message) {
    ...
    var restart = this.createSymbolText(
      "fa fa-fw fa-recycle",
      this.translate("RESTARTMM"),
      buttons["restart-mm-button"]
    );
  ...
  offerReload: function (message) {
    ...
    var reload = this.createSymbolText(
      "fa fa-fw fa-globe",
      this.translate("REFRESHMM"),
      buttons["refresh-mm-button"]
    );
  ...
  loadModulesToUpdate: function () {
    ...
    this.loadList(
      "update-module",
      "moduleInstalled",
      function (parent, modules) {
        for (var i = 0; i < modules.length; i++) {
          var symbol = "fa fa-fw fa-toggle-up";
          var innerWrapper = document.createElement("div");
          innerWrapper.className = "module-line";

          let moduleBox = self.createSymbolText(
            symbol,
            modules[i].name,
            function (event) {
          ...
          if (modules[i].updateAvailable) {
            let moduleBox = self.createSymbolText(
              ...
              self.translate("UPDATE_AVAILABLE")
            );
            innerWrapper.appendChild(moduleBox);
          }
  ...
  createMenuElement: function (content, menu, $insertAfter) {
    ...
    $item = $("<div>")
      .attr("id", `${content.id}-button`)
      .addClass(`menu-element button ${menu}-menu`);
    let $mcmIcon = $("<span>")
      .addClass(`fa fa-fw fa-${content.icon}`)
    ...
    if (content.type === "menu") {
      ...
      let $mcmArrow = $("<span>")
        .addClass("fa fa-fw fa-angle-right")
        .attr("aria-hidden", "true");
            ...
            content.content,
            {
              payload: Object.assign(
                ...
                content.content == undefined
                  ? {}
                  : typeof content.content.payload === "string"
      ...
      $item = $("<input>")
        .addClass(`menu-element ${menu}-menu medium`)
        .attr({
          id: `${content.id}-input`,
          type: "text",
    ...
    if (
      (!window.location.hash && menu !== "main") ||
      (window.location.hash &&
        window.location.hash.substring(1) !== menu + "-menu")
    ) {
...
var buttons = {
  ...
  "image-button": function () {
    window.location.hash = "image-menu";
  },
  ...
  "settings-button": function () {
    ...
    var panic = self.createSymbolText(
      "fa fa-life-ring",
      self.translate("PANIC"),
      function () {
        self.setStatus("none");
      }
    );
  ...
  "back-button": function () {
    ...
    if ($(window.location.hash.replace("-menu", "-button")).data("parent")) {
      window.location.hash =
        $(window.location.hash.replace("-menu", "-button")).data("parent") +
        "-menu";
      return;
    }
    window.location.hash = "main-menu";
  ...
  "shut-down-button": function () {
    ...
    var cancel = self.createSymbolText(
      "fa fa-times",
