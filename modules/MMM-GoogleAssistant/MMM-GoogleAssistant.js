...
Module.register("MMM-GoogleAssistant", {
  ...
  start: function () {
    ...
    var callbacks = {
      ...
      myStatus: (status) => {
        ...
        this.sendNotification(
          "ASSISTANT_" + this.myStatus.actual.toUpperCase()
        );
        if (this.config.Extented.useEXT)
          this.EXTActionsOnStatus(this.myStatus.actual);
      },
      EXT: (response) => {
        if (this.config.Extented.useEXT) return this.ExtentedDisplay(response);
      },
      ...
      spotifyStatus: (status) => {
        // try to use spotify callback to unlock screen ...
        if (status) {
          /** Spotify active **/
          ...
          if (
            this.EXT.spotify.player &&
            this.config.Extented.screen.useScreen &&
            !this.displayEXTResponse.working()
          ) {
            if (this.EXT.radioPlayer.play) return;
            this.sendSocketNotification("SCREEN_LOCK", false);
            this.displayEXTResponse.showDivWithAnimatedFlip(
              "EXT_SCREEN_CONTENER"
            );
          }
    ...
    this.assistantResponse = new AssistantResponse(
      this.helperConfig["responseConfig"],
      callbacks
    );
    ...
    if (this.config.Extented.useEXT) {
      ...
      this.config.Extented.spotify.visual.deviceDisplay =
        this.translate("SpotifyListenText");
      this.config.Extented.spotify.visual.SpotifyForGA =
        this.translate("SpotifyForGA");
      ...
      if (this.config.Extented.spotify.useSpotify)
        this.spotify = new Spotify(
          this.config.Extented.spotify.visual,
          callbacks,
          this.config.debug
        );
      this.EXT = this.displayEXTResponse.EXT;
      if (
        this.config.Extented.youtube.useYoutube &&
        this.config.Extented.youtube.useVLC
      )
  ...
  getDom: function () {
    ...
    if (this.config.Extented.useEXT) {
      ...
      if (
        this.config.Extented.spotify.useSpotify &&
        !this.config.Extented.spotify.visual.useBottomBar
      ) {
      ...
      if (
        ...
        this.config.Extented.photos.displayType == "Module"
      ) {
        var GPhotosAPI = document.createElement("div");
        GPhotosAPI.id = "EXT_GPHOTO";
        GPhotosAPI.style.height =
          this.config.Extented.photos.moduleHeight + "px";
      ...
      if (
        !this.config.Extented.screen.useScreen ||
        this.config.Extented.screen.displayStyle != "Text"
      )
      ...
      if (
        !this.config.Extented.screen.useScreen ||
        this.config.Extented.screen.displayStyle == "Text" ||
        !this.config.Extented.screen.displayBar
      )
      ...
      if (
        !this.config.Extented.internet.useInternet ||
        !this.config.Extented.internet.displayPing
      )
        internet.className = "hidden";
  ...
  notificationReceived: function (noti, payload = null, sender = null) {
    this.doPlugin("onNotificationReceived", {
      notification: noti,
      payload: payload
    });
    switch (noti) {
        ...
        if (this.config.Extented.useEXT) {
          ...
          if (this.config.Extented.screen.useScreen) {
            if (this.config.Extented.screen.displayStyle != "Text")
              this.displayEXTResponse.prepareBar();
            if (this.config.Extented.screen.animateBody)
              this.displayEXTResponse.prepareBody();
          }
          if (
            this.config.Extented.spotify.useSpotify &&
            this.config.Extented.spotify.visual.useBottomBar
          )
            this.spotify.prepare();
          if (this.config.Extented.touch.useTouch)
            this.touchScreen(this.config.Extented.touch.mode);
        }
        this.assistantResponse.prepareGA();
        this.Loading();
        if (
          this.config.Extented.useEXT &&
          this.config.Extented.photos.usePhotos &&
          (this.config.Extented.photos.displayType == "Background" ||
            this.config.Extented.photos.displayType == "Module")
        ) {
          setTimeout(() => {
            if (this.config.Extented.photos.useGooglePhotosAPI)
              this.displayEXTResponse.showBackgroundGooglePhotoAPI();
            else
      ...
      case "EXT_LOCK" /** screen lock **/:
        if (
          ...
          this.displayEXTResponse.hideDivWithAnimatedFlip(
            "EXT_SCREEN_CONTENER"
          );
          this.Informations("information", {
            message: "ScreenLock",
  ...
  socketNotificationReceived: function (noti, payload) {
    ...
    switch (noti) {
            ...
            payload.forEach((npm) => {
              this.sendNotification("SHOW_ALERT", {
                ...
                message:
                  ...
                  " -> v" +
                  npm.latest,
                title: this.translate("UPDATE_NOTIFICATION_MODULE", {
      ...
      case "NOT_INITIALIZED":
        ...
        this.assistantResponse.showError(
          this.translate(payload.message, { VALUES: payload.values })
        );
      ...
      case "SCREEN_BAR":
        if (this.config.Extented.screen.displayStyle != "Text") {
          let value =
            (100 - (payload * 100) / this.config.Extented.screen.delay) / 100;
          let timeOut = moment(
            new Date(this.config.Extented.screen.delay - payload)
          ).format("mm:ss");
          this.displayEXTResponse.bar.animate(value, {
            step: (state, bar) => {
              bar.path.setAttribute("stroke", state.color);
              bar.setText(
                this.config.Extented.screen.displayCounter ? timeOut : ""
              );
      ...
      case "SCREEN_PRESENCE":
        if (this.config.Extented.screen.userPresenceNotification)
        ...
        if (
          this.userPresence &&
          this.config.Extented.screen.displayLastPresence
        ) {
      ...
      case "SCREEN_POWER":
        ...
        if (payload)
          this.Informations("information", { message: "ScreenPowerOn" });
      ...
      case "INTERNET_DOWN":
        ...
        this.Informations("warning", {
          message: "InternetDown",
          values: FormatedSince
        });
        break;
      ...
      case "INTERNET_AVAILABLE":
        ...
        let FormatedMessage =
          (DateDiff.day
            ? DateDiff.day +
              (DateDiff.day > 1
                ? this.DateTranslate.days
                : this.DateTranslate.day)
            : "") +
          (DateDiff.hour
            ? DateDiff.hour +
              (DateDiff.hour > 1
                ? this.DateTranslate.hours
                : this.DateTranslate.hour)
            : "") +
          (DateDiff.min
            ? DateDiff.min +
              (DateDiff.min > 1
                ? this.DateTranslate.minutes
                : this.DateTranslate.minute)
            : "") +
          DateDiff.sec +
          (DateDiff.sec > 1
            ? this.DateTranslate.seconds
            : this.DateTranslate.second);
        this.Informations("information", {
          message: "InternetAvailable",
          values: FormatedMessage
        });
        break;
      case "INTERNET_PING":
  ...
  assistantActivate: function (payload) {
    if (this.myStatus.actual != "standby" && !payload.force)
      return logGA("Assistant is busy.");
    this.clearAliveTimers();
    if (this.myStatus.actual == "continue")
      this.assistantResponse.showTranscription(this.translate("GAContinue"));
        ...
        input.payload.commands.forEach((command) => {
          command.execution.forEach((exec) => {
            ...
            if (exec.command == "action.devices.commands.SetVolume") {
              if (
                this.config.Extented.useEXT &&
                this.config.Extented.volume.useVolume
              ) {
                logGA("Volume Control:", exec.params.volumeLevel);
                this.sendSocketNotification(
                  "VOLUME_SET",
                  exec.params.volumeLevel
                );
              }
            }
          });
  ...
  doCommand: function (commandId, originalParam, from) {
    ...
    var param =
      typeof originalParam == "object"
        ? Object.assign({}, originalParam)
        : originalParam;
    ...
    if (command.hasOwnProperty("notificationExec")) {
      ...
      if (ne.notification) {
        var fnen =
          typeof ne.notification == "function"
            ? ne.notification(param, from)
            : ne.notification;
        var nep = ne.payload
          ? typeof ne.payload == "function"
            ? ne.payload(param, from)
            : ne.payload
          : null;
    ...
    if (command.hasOwnProperty("moduleExec")) {
      ...
      var mo =
        typeof me.module == "function" ? me.module(param, from) : me.module;
      ...
      if (typeof me.exec == "function") {
        MM.getModules().enumerate((mdl) => {
          if (m.length == 0 || m.indexOf(mdl.name) >= 0) {
            logGA(
              `Command ${commandId} is executed (moduleExec) for :`,
              mdl.name
            );
  ...
  ExtentedDisplay: function (response) {
    ...
    if (
      response.screen &&
      (response.screen.links.length > 0 || response.screen.photos.length > 0)
    ) {
  ...
  tbHide: function (command, handler) {
    ...
    if (handler.args) {
        ...
        if (m.name == handler.args) {
          ...
          if (m.hidden)
            return handler.reply(
              "TEXT",
              handler.args + this.translate("HIDE_ALREADY")
            );
          if (m.lockStrings.length > 0) {
            m.lockStrings.forEach((lock) => {
              if (lock == "TB_EXT") {
                ...
                if (m.lockStrings.length == 0) {
                  ...
                  handler.reply(
                    "TEXT",
                    handler.args + this.translate("HIDE_DONE")
                  );
                }
              }
            });
            if (!unlock)
              return handler.reply(
                "TEXT",
                handler.args + this.translate("HIDE_LOCKED")
              );
          } else {
            m.hide(500, { lockString: "TB_EXT" });
      ...
      if (!found)
        handler.reply("TEXT", this.translate("MODULE_NOTFOUND") + handler.args);
      ...
      MM.getModules().enumerate((m) => {
        if (m.name == handler.args) {
          ...
          if (!m.hidden)
            return handler.reply(
              "TEXT",
              handler.args + this.translate("SHOW_ALREADY")
            );
          if (m.lockStrings.length > 0) {
            m.lockStrings.forEach((lock) => {
              if (lock == "TB_EXT") {
                ...
                if (m.lockStrings.length == 0) {
                  ...
                  handler.reply(
                    "TEXT",
                    handler.args + this.translate("SHOW_DONE")
                  );
                }
              }
            });
            if (!unlock)
              return handler.reply(
                "TEXT",
                handler.args + this.translate("SHOW_LOCKED")
              );
          } else {
            m.show(500, { lockString: "TB_EXT" });
  ...
  tbEXT: function (command, handler) {
    if (handler.args) {
      ...
      var regexp =
        /^((http(s)?):\/\/)(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/;
  ...
  tbVolume: function (command, handler) {
    if (handler.args) {
      ...
      if ((!value && value != 0) || value < 0 || value > 100)
        return handler.reply("TEXT", "/volume [0-100]");
  ...
  tbSpotify: function (command, handler) {
    if (handler.args) {
      ...
      if (args[0] == "volume") {
        if (args[1]) {
          if (isNaN(args[1]))
            return handler.reply("TEXT", "Must be a number ! [0-100]");
      ...
      if (args[0] == "to") {
        if (args[1]) {
          handler.reply(
            "TEXT",
            "Spotify TRANSFER to: " + params[1] + " (if exist !)"
          );
      ...
      handler.reply(
        "TEXT",
        "Need Help for /spotify commands ?\n\n\
  ...
  *to*: Transfert music to another device (case sensitive)\
  ...
  EXTActionsOnStatus: function (status) {
    ...
    switch (status) {
      ...
      case "think":
        //this.EXT.speak = true
        if (this.config.Extented.screen.useScreen && !this.EXT.locked)
        ...
        if (this.config.Extented.youtube.useYoutube) {
          ...
          else if (this.displayEXTResponse.player)
            this.displayEXTResponse.player.command(
              "setVolume",
              this.config.Extented.youtube.minVolume
            );
        }
        if (
          ...
          this.sendSocketNotification(
            "SPOTIFY_VOLUME",
            this.config.Extented.spotify.player.minVolume
          );
        }
        if (this.EXT.radioPlayer.play)
          this.displayEXTResponse.radio.volume = 0.1;
        break;
      ...
      case "standby":
        ...
        if (this.config.Extented.youtube.useYoutube) {
          if (this.config.Extented.youtube.useVLC)
            this.sendSocketNotification(
              "YT_VOLUME",
              this.config.Extented.youtube.maxVolume
            );
          else if (this.displayEXTResponse.player)
            this.displayEXTResponse.player.command(
              "setVolume",
        ...
        if (
          ...
          this.sendSocketNotification(
            "SPOTIFY_VOLUME",
            this.EXT.spotify.targetVolume
          );
        }
        this.EXT.spotify.forceVolume = false;
        if (this.EXT.radioPlayer.play)
          this.displayEXTResponse.radio.volume = 0.6;
        break;
      case "reply":
        if (this.displayEXTResponse.working())
          this.displayEXTResponse.showDisplay();
  ...
  sendWelcome() {
    if (
      this.config.Extented.welcome.useWelcome &&
      this.config.Extented.welcome.welcome
    ) {
      this.assistantActivate(
        {
          type: "TEXT",
          key: this.config.Extented.welcome.welcome,
          chime: false
        },
        Date.now()
      );
  ...
  checkStyle: function () {
    ...
    if (!found) {
      console.log(
        "[GA:EXT] displayStyle Error ! [" +
          this.config.Extented.screen.displayStyle +
          "]"
      );
      this.config.Extented.screen = Object.assign(
        {},
        this.config.Extented.screen,
        { displayStyle: "Text" }
      );
    }
  },
  ...
  initializeVolumeVLC: function () {
    ...
    try {
      ...
      if (typeof valueMin === "number" && valueMin >= 0 && valueMin <= 100)
        ...
        console.error(
          "[GA:EXT] config.youtube.minVolume error! Corrected with 30"
        );
    ...
    try {
      ...
      if (typeof valueMax === "number" && valueMax >= 0 && valueMax <= 100)
        ...
        console.error(
          "[GA:EXT] config.youtube.maxVolume error! Corrected with 100"
        );
    ...
    if (this.config.Extented.spotify.useSpotify) {
      ...
      switch (command) {
        ...
        case "SHUFFLE":
          this.sendSocketNotification(
            "SPOTIFY_SHUFFLE",
            !this.EXT.spotify.shuffle
          );
          break;
        case "REPEAT":
          this.sendSocketNotification(
            "SPOTIFY_REPEAT",
            this.EXT.spotify.repeat == "off" ? "track" : "off"
          );
          break;
        case "TRANSFER":
        ...
        case "SEARCH":
          ...
          var type = null;
          if (searchType[0] == this.translate("SpotifySearchTypePlaylist"))
            type = "playlist";
          else if (searchType[0] == this.translate("SpotifySearchTypeAlbum"))
            type = "album";
          else if (searchType[0] == this.translate("SpotifySearchTypeTrack"))
            type = "track";
          else if (searchType[0] == this.translate("SpotifySearchTypeArtist"))
            type = "artist";
          if (type) {
  ...
  stopCommand: function () {
    ...
    if (this.EXT.spotify.player) {
      if (this.config.Extented.spotify.player.usePause)
        this.sendSocketNotification("SPOTIFY_PAUSE");
  ...
  showGooglePhotos() {
    if (!this.config.Extented.useEXT)
      return this.Informations("warning", { message: "EXTNotActivated" });
    if (!this.config.Extented.photos.usePhotos)
      return this.Informations("warning", { message: "PhotosNotActivated" });
    if (!this.config.Extented.photos.useGooglePhotosAPI)
      return this.Informations("warning", { message: "GPhotosNotActivated" });
    if (this.config.Extented.photos.displayType == "Background")
      return this.Informations("warning", {
        message: "GPhotosBckGrndActivated"
      });
    if (this.config.Extented.photos.displayType == "Module")
      return this.Informations("warning", {
        message: "GPhotosModuleActivated"
      });
    if (this.config.Extented.photos.displayType != "Recipe")
      return this.Informations("warning", {
        message: "GPhotosRecipeNotActivated"
      });
  ...
  Informations(type, info) {
    ...
    if (informationsType.indexOf(type) == -1) {
      ...
      return this.Informations("warning", {
        message: "Core Information: Display Type Error!"
      });
    }
    if (!info.message) {
      // should not happen
      ...
      return this.Informations("warning", {
        message: "Core Information: no message!"
      });
    }
    ...
    this.InformationsBuffer(
      this.Infos.buffer[0].type,
      this.Infos.buffer[0].info
    );
  },
  ...
  InformationsBuffer: function (type, info) {
    ...
    logGA(type + ":", info);
    if (type == "warning" && this.config.responseConfig.useChime)
      this.assistantResponse.infoWarning.src =
        this.assistantResponse.resourcesDir +
        this.assistantResponse.chime["warning"];
    if (type == "information" && !this.config.responseConfig.useInformations)
      return;
  ...
  InformationHidden: function () {
    ...
    this.assistantResponse.infosDiv.addEventListener(
      "animationend",
      (e) => {
  ...
  InformationShow: function () {
    ...
    this.assistantResponse.infosDiv.classList.remove(
      "hidden",
  ...
  Loading: function () {
    ...
    this.assistantResponse.showTranscription(
      this.translate("GALoading") + " MMM-GoogleAssistant"
    );
  ...
  Version(version) {
    this.assistantResponse.showTranscription(
      "MMM-GoogleAssistant v" +
        version.version +
        " (" +
        version.rev +
        ") ©bugsounet " +
        this.translate("GAReady")
    );
    this.assistantResponse.fullscreen(true, null, false);
